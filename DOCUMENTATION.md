...
Die Grafik zeigt ein eeispielhaften Verlauf an dem ein gutes Trainingssenario gemessen werden kann. Der erste Graph Lesson zeigt beim curriculum training den Fortschritt an, welcher für unser Training nicht genutzt wurde. Der Graph des Cumulative Reward ist die durchschnittliche Belohnung über alle Angents in einer Epoche. Da in unserem Senario der Erfolg des einen Agents den Misserfolg des Gegenspieler (adversarial selfplay) bedeutet, wird dieser Graph bei uns allerdings immer nur den halben Reward des Gewinner anzeigen. Die Episodenlänge sollte bei uns mit der Zeit, anders als in diesem Beispiel hier abnehmen da die beiden Agents wissen sollten was sie tun und nicht beide in irgend einer Ecke fest hängen sollten. Value Loss beschreibt wie genau ein Agent in der Lage ist sein Erfolg vorherzusagen. 
